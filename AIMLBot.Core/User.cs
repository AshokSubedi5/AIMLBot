...
namespace AIMLBot.Core
{
    ...
    public class User
    {
		...
		/// </summary>
        public AIMLBot.Core.Utils.SettingsDictionary Predicates;
		...
		public User(string UserID, AIMLBot.Core.Bot bot)
		{
            if (UserID.Length > 0)
            {
                this.id = UserID;
                this.bot = bot;
                this.Predicates = new AIMLBot.Core.Utils.SettingsDictionary(this.bot);
